package com.geppetto.demo1.service.serviceimpl;
...
import com.geppetto.demo1.exception.EntityNotFoundException;
...
public class StudentServiceImpl implements StudentService{
    ...
    @Override
    public List<StudentDto> search(Map<String, String> allParams) {
        Query query = new BasicQuery(constructQuery(allParams));
        List<Student> studentEntities= mongoTemplate.find(query, Student.class);
        List<StudentDto> studentDtoList = new ArrayList<>();
        for (Student studentEntity : studentEntities) {
             StudentDto studentDto = new StudentDto();
             BeanUtils.copyProperties(studentEntity, studentDto);
